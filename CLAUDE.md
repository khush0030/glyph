...
- **Analysis LLM (cloud default):** **Claude Haiku 4.5** (`claude-haiku-4-5`, ~2¢/meeting). Sonnet 4.6 as an optional "deep notes" toggle.
...
- **Storage:** local **SQLite**. No web backend — the desktop app holds API keys in the macOS Keychain and calls Scribe / Anthropic / Google / Asana directly.
...
- `NoteGenerator` — transcript + scratch → markdown. Cloud: Haiku 4.5. Local: Ollama.
...
## Hard rules
1. **Keys live in the macOS Keychain only** — never in the repo, never in plaintext config. Scribe/Anthropic/Google/Asana tokens all go through Keychain.
